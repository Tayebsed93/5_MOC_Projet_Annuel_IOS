...
@objc public class PermissionScope: UIViewController, CLLocationManagerDelegate, UIGestureRecognizerDelegate, CBPeripheralManagerDelegate {
    ...
    override public func viewWillLayoutSubviews() {
        ...
        // Set frames
        let x = (screenSize.width - ConstantsPhoto.UI.contentWidth) / 2
        
        let dialogHeight: CGFloat
        switch self.configuredPermissions.count {
        case 2:
            dialogHeight = ConstantsPhoto.UI.dialogHeightTwoPermissions
        case 3:
            dialogHeight = ConstantsPhoto.UI.dialogHeightThreePermissions
        default:
            dialogHeight = ConstantsPhoto.UI.dialogHeightSinglePermission
        }
        
        let y = (screenSize.height - dialogHeight) / 2
        contentView.frame = CGRect(x:x, y:y, width:ConstantsPhoto.UI.contentWidth, height:dialogHeight)
    ...
    public func statusLocationAlways() -> PermissionStatus {
        ...
        case .authorizedWhenInUse:
            ...
            if defaults.bool(forKey: ConstantsPhoto.NSUserDefaultsKeys.requestedInUseToAlwaysUpgrade) {
                return .unauthorized
            } else {
                return .unknown
            }
    ...
    public func requestLocationAlways() {
        let hasAlwaysKey:Bool = !Bundle.main
            .object(forInfoDictionaryKey: ConstantsPhoto.InfoPlistKeys.locationAlways).isNil
        assert(hasAlwaysKey, ConstantsPhoto.InfoPlistKeys.locationAlways + " not found in Info.plist.")
        
        let status = statusLocationAlways()
        switch status {
        case .unknown:
            if CLLocationManager.authorizationStatus() == .authorizedWhenInUse {
                defaults.set(true, forKey: ConstantsPhoto.NSUserDefaultsKeys.requestedInUseToAlwaysUpgrade)
    ...
    public func requestLocationInUse() {
        let hasWhenInUseKey :Bool = !Bundle.main
            .object(forInfoDictionaryKey: ConstantsPhoto.InfoPlistKeys.locationWhenInUse).isNil
        assert(hasWhenInUseKey, ConstantsPhoto.InfoPlistKeys.locationWhenInUse + " not found in Info.plist.")
    ...
    public func statusNotifications() -> PermissionStatus {
        ...
        if let settingTypes = settings?.types , settingTypes != UIUserNotificationType() {
            ...
            if defaults.bool(forKey: ConstantsPhoto.NSUserDefaultsKeys.requestedNotifications) {
                return .unauthorized
            } else {
                return .unknown
            }
    ...
    @objc func finishedShowingNotificationPermission () {
        ...
        defaults.set(true, forKey: ConstantsPhoto.NSUserDefaultsKeys.requestedNotifications)
        defaults.synchronize()
    ...
    fileprivate var askedBluetooth:Bool {
        get {
            return defaults.bool(forKey: ConstantsPhoto.NSUserDefaultsKeys.requestedBluetooth)
        }
        set {
            defaults.set(newValue, forKey: ConstantsPhoto.NSUserDefaultsKeys.requestedBluetooth)
            defaults.synchronize()
        }
    }
    ...
    fileprivate func triggerMotionStatusUpdate() {
        ...
        defaults.set(true, forKey: ConstantsPhoto.NSUserDefaultsKeys.requestedMotion)
        defaults.synchronize()
    ...
    fileprivate var askedMotion:Bool {
        get {
            return defaults.bool(forKey: ConstantsPhoto.NSUserDefaultsKeys.requestedMotion)
        }
        set {
            defaults.set(newValue, forKey: ConstantsPhoto.NSUserDefaultsKeys.requestedMotion)
            defaults.synchronize()
        }
    }
