...
struct defaultssKeys {
    static let league_id = "league_id"
    static let urlResult = "urlResult"
}

class ResultMatch: UIViewController, UICollectionViewDelegate, UICollectionViewDataSource, NVActivityIndicatorViewable {
                            ...
                            for i in 0..<nb {
                                if let match_hometeam_name = json![i]["match_hometeam_name"], let match_awayteam_name = json![i]["match_awayteam_name"], let match_hometeam_score = json![i]["match_hometeam_score"], let match_awayteam_score = json![i]["match_awayteam_score"], let match_date = json![i]["match_date"], let match_time = json![i]["match_time"], let league_name = json![i]["league_name"] {
                                    ...
                                    if match_hometeam_name == self.passnameclub || match_awayteam_name == self.passnameclub  {
                                        ...
                                        self.calendrierStruct.append(calendrierr.init(match_hometeam_name: match_hometeam_name, match_awayteam_name: match_awayteam_name, match_hometeam_score: match_hometeam_score, match_awayteam_score: match_awayteam_score, date: date_array, match_time: match_time, league_name: league_name))
...
extension ResultMatch: PopupDelegate {
